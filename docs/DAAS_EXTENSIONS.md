...
| `qc-lots-autoname` | `qc_lots.items.create` | filter | Auto-fills `lot_code` = `<CODE>-NNNN` (running per-product counter; CODE from `products.code`) when the operator leaves it blank; keeps supplied codes. Requires a `code` field on products. Snapshot: `docs/daas/qc-lots-autoname.js`. |

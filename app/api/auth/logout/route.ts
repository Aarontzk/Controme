...
// Active DaaS scope cookie set by DaaSProviderWrapper — must be cleared on
// logout so a stale scope is not forwarded as X-Resource-Uri on next login.
const RESOURCE_URI_COOKIE_NAME = 'daas_resource_uri';
...
async function performLogout(
  ...
  // Clear the provider tracking cookie and the active DaaS scope cookie
  cookieStore.delete(PROVIDER_COOKIE_NAME);
  cookieStore.delete(RESOURCE_URI_COOKIE_NAME);
